...
# async def read_root():
#     """Returns the content of the Homepage.html file."""
#     try:
#         with open("Homepage.html", "r", encoding="utf-8") as file:  # Added encoding for better compatibility
#             html_content = file.read()
#         return html_content  # No need for HTMLResponse explicitly, FastAPI will handle it
#     except FileNotFoundError:
#         raise HTTPException(status_code=404, detail="Homepage.html not found")
#     except Exception as e:
#         raise HTTPException(status_code=500, detail=f"An error occurred: {str(e)}")


# @app.post("/process")
# async def send_complete_video():
#     """Processes the video and returns it as a response."""
#     AVI_FILE_PATH = os.path.join(os.getcwd(), "output.mp4")  # Ensuring cross-platform file path handling
    
#     try:
#         # Run the main processing logic (use async if it's blocking)
#         await main()  # Assuming `main` is an async function, otherwise use `loop.run_in_executor`
        
#         if os.path.exists(AVI_FILE_PATH):
#             return FileResponse(AVI_FILE_PATH, media_type="video/mp4", filename="output.mp4")
#         else:
